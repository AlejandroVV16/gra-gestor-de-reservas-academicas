...
import { getAuditorios, getDisponibilidad } from '../api/auditoriosApi'
...
import { MOCK_FACULTADES, MOCK_TIPOS_EVENTO, MOCK_RESERVAS } from '../data/mockData'
...
export default function EditarReserva() {
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        ...
        setForm({
          fecha: data.request_date || data.event_start?.split('T')[0] || '',
          horaInicio: data.event_start ? new Date(data.event_start).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: 'UTC' }) : '',
          horaFin: data.event_end ? new Date(data.event_end).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: 'UTC' }) : '',
          auditorioId: data.auditorium_id || '',
          encargado: data.responsible_person || '',
          facultad: '',
          tipoEvento: '',
          nombreEvento: data.event_name || '',
          descripcion: data.notes || '',
          personas: String(data.attendees_count || ''),
          equipos: [],
          personalTI: [],
          usarSecciones: false,
          secciones: [],
        ...
        addToast({ tipo: 'error', mensaje: 'No se pudo cargar la reserva' })
      } finally {
        setCargando(false)
      }
    }
  ...
  const esDivisible  = false

  const capacidadEfectiva = auditorioSel?.capacity || 999
            ...
            {auditorioSel ? (
              <>
                ...
                <div className="space-y-1.5 text-sm">
                  <div className="flex gap-2"><span className="text-gray-400 w-24">Sede:</span><span className="font-medium">{auditorioSel.location || '—'}</span></div>
